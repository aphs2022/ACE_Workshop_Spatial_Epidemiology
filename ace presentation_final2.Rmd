...
setwd("C:/Users/rpaul9/Dropbox (UNC Charlotte)/ACE_Workshop/ACE_Workshop_Spatial")
...
Let's map these fatal West Nile Virus categories
...
1. Rurality/Urbanicity: Binary variable - Rural/Urban
4. Poverty Rates: Poverty
5. Percent of population having less than high school education
6. Computing expected death counts as an offset variable

```{r}

### Create a rural/urban variable using RUCA Codes ###
Metropolitan <- as.numeric(case.data$ruca3gp == "Metropolitan")
## How many Metropolitan Counties? ## 
table(Metropolitan)
## Add this variable to case.data ###
case.data$Metropolitan <- Metropolitan

## Prepare for INLA-R Runs ###
tab1 <- CreateTableOne (vars = c ("Metropolitan", "lesshighschool",
                                  "Poverty", "EC"),
                        data = case.data)
...
formula1 <- TotalCases ~ Metropolitan + lesshighschool + Poverty
...
## Summary Statistics ##
results.inla <- summary(dat.inla)
## Getting Estimated Coefficients ##
regression.coefficients <- results.inla$fixed
## Calculating IRR ##
Table1 <- regression.coefficients[, c(1,3:5)]## Retain only columns 1, 3 to 5
IRR.Table1 <- exp(Table1)
View(IRR.Table1)
## Rounding to Two Decimal Places ##
View(round(IRR.Table1, 2))
## Save as .csv file ##
write.csv(round(IRR.Table1, 2), file = "nonspatial.NB.csv")
### Obtaining Watanabe–Akaike Information Criterion ###
results.inla$waic$waic
...
```{r}
pa.tr <- readOGR("PaCounty2023_08.shp")
```
...
3. Non-standardized and Row standardized matrices
```{r}
W.pa <- nb2mat(pa.adj, style = "B") 
W.pa.rs <- nb2mat(pa.adj, style = "W") 

#wt2 = read.gal("3102counties_wt.gal",override.id = TRUE)
#wt2.listw = nb2listw(wt2, style = "W", zero.policy = TRUE)
#class(wt2.listw)
```
...
pa.formula <- TotalCases ~ Metropolitan + lesshighschool + Poverty + f(ID, model = "bym", graph = W.pa.rs)
...
summary(pa.bym)
## Summary Statistics ##
results.inla.bym <- summary(pa.bym)
## Getting Estimated Coefficients ##
regression.coefficients.bym <- results.inla.bym$fixed
## Calculating IRR ##
Table1.bym <- regression.coefficients.bym[, c(1,3:5)]## Retain only columns 1, 3 to 5
IRR.Table1.bym <- exp(Table1.bym)
View(IRR.Table1.bym)
## Rounding to Two Decimal Places ##
View(round(IRR.Table1.bym, 2))
## Save as .csv file ##
write.csv(round(IRR.Table1.bym, 2), file = "spatial.NB.csv")
### Obtaining Watanabe–Akaike Information Criterion ###
results.inla.bym$waic$waic
...
```{r}
stformula <- count~ years65 + black + poverty + obesity + metro + f(fips, timeID, model = "iid")
...
```{r}
model<-inla(stformula,data = data.all, 
             family = "nbinomial", E = ec, #marginal distribution for the outcome, expected count
             control.fixed = prior.fixed,
             control.compute = list(waic=T, return.marginals.predictor=TRUE), # compute DIC or not?
             control.predictor = list(link=1, compute=TRUE)) #estimate predicted values & their marginals or not?
